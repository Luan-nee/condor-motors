...
class ProformaVentaApi {
  ...
  Future<Map<String, dynamic>> convertirProformaAVenta({
    ...
    try {
      debugPrint('🔄 Intentando convertir proforma $proformaId a venta con datos: $datosVenta');
      ...
      debugPrint('✅ Proforma $proformaId convertida exitosamente');
      return response;
    } catch (e) {
      // Verificar si es un error de JSON
      if (e.toString().contains('JSON') || e.toString().contains('empty')) {
        debugPrint('⚠️ Error de formato JSON al convertir proforma: $e');
        
        // Intentar verificar si la proforma se convirtió a pesar del error
        try {
          debugPrint('🔍 Verificando si la proforma $proformaId se convirtió a pesar del error');
          ...
          if (proforma != null && proforma.estado == proforma_model.EstadoProforma.convertida) {
            debugPrint('✅ La proforma $proformaId ya fue convertida exitosamente a pesar del error de comunicación');
            return {'status': 'success', 'message': 'Proforma convertida exitosamente', 'data': {'id': proformaId}};
          }
        } catch (verificationError) {
          debugPrint('❌ Error al verificar estado de la proforma: $verificationError');
        }
      }
      
      debugPrint('⚠️ El endpoint convertirProformaAVenta puede no existir o falló: $e');
      ...
      try {
        ...
        try {
          ...
          if (proforma == null) {
            throw Exception('No se pudo obtener la información de la proforma');
          }
        } catch (e) {
          debugPrint('❌ Error al obtener la proforma para conversión: $e');
          throw Exception('Error al obtener datos de la proforma: $e');
        }

        // Asegurarse de que la proforma esté en estado pendiente
        if (proforma.estado != proforma_model.EstadoProforma.pendiente) {
          if (proforma.estado == proforma_model.EstadoProforma.convertida) {
            debugPrint('ℹ️ La proforma ya fue convertida anteriormente');
            return {
              'status': 'success',
              'message': 'Esta proforma ya fue convertida a venta anteriormente',
              'data': {'id': proformaId}
            };
          } else {
            throw Exception('La proforma no está en estado pendiente. Estado actual: ${proforma.estado.toText()}');
          }
        }

        // Preparar datos para la API de ventas
        final Map<String, dynamic> ventaData = <String, dynamic>{
          'productos': datosVenta['productos'] ?? proforma.detalles.map((detalle) => {
            'productoId': detalle.productoId,
            'cantidad': detalle.cantidad,
            'precio': detalle.precioUnitario,
          }).toList(),
          'cliente': datosVenta['cliente'] ?? (proforma.cliente ?? {'nombre': 'Cliente sin nombre'}),
          'metodoPago': datosVenta['metodoPago'] ?? 'EFECTIVO',
          'tipoDocumento': datosVenta['tipoDocumento'] ?? 'BOLETA',
          'total': datosVenta['total'] ?? proforma.total,
          'empleadoId': proforma.empleadoId,
          'sucursalId': proforma.sucursalId,
          'observaciones': 'Generado a partir de proforma #${proforma.id}',
        };
        
        debugPrint('🔄 Creando venta a partir de proforma con datos: $ventaData');
        ...
        if (ventaResponse['status'] == 'success') {
          debugPrint('✅ Venta creada exitosamente, actualizando estado de proforma');
          try {
            ...
            debugPrint('⚠️ Error al actualizar el estado de la proforma: $e. La venta se creó pero el estado de la proforma puede no reflejarlo.');
          }
        } else {
          throw Exception('Error al crear la venta: ${ventaResponse['message'] ?? 'Respuesta inválida del servidor'}');
        }
        ...
        debugPrint('✅ Proforma $proformaId convertida a venta exitosamente mediante API alternativa');
        return response;
      } catch (ventaError) {
