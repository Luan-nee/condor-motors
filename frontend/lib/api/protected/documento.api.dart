...
class TipoDocumento {
  ...
  @override
  String toString() =>
      'TipoDocumento(id: $id, nombre: $nombre, codigo: $codigo)';
}
...
class DocumentoApi {
  ...
  void _procesarInformacion(Map<String, dynamic> data) {
    try {
      ...
      if (data.containsKey('tiposDocFacturacion') &&
          data['tiposDocFacturacion'] is List) {
  ...
  Future<int> getBoletaId(String sucursalId) async {
    ...
    if (_boletaId == null) {
      Logger.warn(
          'No se pudo obtener ID para Boleta, usando valor por defecto (4)');
  ...
  Future<int> getFacturaId(String sucursalId) async {
    ...
    if (_facturaId == null) {
      Logger.warn(
          'No se pudo obtener ID para Factura, usando valor por defecto (3)');
  ...
  Future<int> getGravadoTaxId(String sucursalId) async {
    ...
    if (_gravadoId == null) {
      Logger.warn(
          'No se pudo obtener ID para Tax Gravado, usando valor por defecto (4)');
      return 4; // Valor por defecto basado en la respuesta API
    }
