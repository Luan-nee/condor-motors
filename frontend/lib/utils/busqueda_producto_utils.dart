...
class BusquedaProductoUtils {
  ...
  static bool esCategoriaTodos(String categoria) {
    ...
    final String categoriaLC = categoria.trim().toLowerCase();
    final bool esTodos = categoriaLC == 'todas' || categoriaLC == 'todos' || categoriaLC == 'all';
    if (esTodos) {
  ...
  static List<Producto> convertirAProductos(List<Map<String, dynamic>> productos) {
    return productos.map((Map<String, dynamic> p) => Producto(
      ...
      precioOferta: p['precioLiquidacion'] is num ? (p['precioLiquidacion'] as num).toDouble() : null,
      liquidacion: p['enLiquidacion'] == true || p['liquidacion'] == true,
      // Añadir información sobre promociones de descuento
      cantidadMinimaDescuento: p['cantidadMinima'] is int ? p['cantidadMinima'] : null,
      cantidadGratisDescuento: p['cantidadGratis'] is int ? p['cantidadGratis'] : null,
      porcentajeDescuento: p['descuentoPorcentaje'] is int ? p['descuentoPorcentaje'] : null,
      // Campos opcionales
  ...
  static List<Map<String, dynamic>> convertirAMapas(List<Producto> productos) {
    return productos.map((Producto p) => <String, Object?>{
      'id': p.id,
      'codigo': p.sku,
      'nombre': p.nombre,
      ...
      'tienePromocionGratis': p.cantidadGratisDescuento != null && p.cantidadGratisDescuento! > 0,
      'tieneDescuentoPorcentual': p.porcentajeDescuento != null && p.porcentajeDescuento! > 0 && 
                                p.cantidadMinimaDescuento != null && p.cantidadMinimaDescuento! > 0,
      ...
      'tienePromocion': p.liquidacion || 
                       ...
                       (p.porcentajeDescuento != null && p.porcentajeDescuento! > 0 && 
                        p.cantidadMinimaDescuento != null && p.cantidadMinimaDescuento! > 0),
    }).toList();
  }
  ...
  static List<Map<String, dynamic>> filtrarPorTipoDescuento(
    ...
    if (debugMode) {
      debugPrint('🔍 Aplicando filtro de tipo de descuento: $tipoDescuento a ${productos.length} productos');
    }
    ...
    final List<Map<String, dynamic>> productosFiltrados = productos.where((Map<String, dynamic> producto) {
      ...
      switch (tipoDescuento) {
        ...
        case TipoDescuento.promoGratis:
          ...
          if (debugMode && !cumpleFiltro) {
            ...
            if (producto['cantidadMinima'] != null && producto['cantidadGratis'] != null) {
              debugPrint('⚠️ Producto ${producto['nombre']} tiene cantidadMinima=${producto['cantidadMinima']} y cantidadGratis=${producto['cantidadGratis']} pero tienePromocionGratis=${producto['tienePromocionGratis']}');
            }
          }
          break;
        case TipoDescuento.descuentoPorcentual:
          ...
          if (debugMode && !cumpleFiltro) {
            // Revisar por qué no cumple para posible diagnóstico
            if (producto['cantidadMinima'] != null && producto['descuentoPorcentaje'] != null) {
              debugPrint('⚠️ Producto ${producto['nombre']} tiene cantidadMinima=${producto['cantidadMinima']} y descuentoPorcentaje=${producto['descuentoPorcentaje']} pero tieneDescuentoPorcentual=${producto['tieneDescuentoPorcentual']}');
            }
          }
          break;
        default:
    ...
    if (debugMode) {
      debugPrint('✅ Filtro aplicado. Resultado: ${productosFiltrados.length} productos cumplen el filtro $tipoDescuento');
    }
  ...
  static List<Map<String, dynamic>> filtrarProductos({
    ...
    if (esFormatoCompatible(productos)) {
      try {
        ...
        final List<Producto> productosFormateados = convertirAProductos(productos);
        
        // Usar la función centralizada de filtrado
        final List<Producto> productosFiltrados = ProductosUtils.filtrarProductos(
        ...
        final List<Map<String, dynamic>> resultadosFiltrados = convertirAMapas(productosFiltrados);
        
        // Aplicar filtrado adicional por tipo de descuento
        return filtrarPorTipoDescuento(resultadosFiltrados, tipoDescuento, debugMode: debugMode);
      } catch (e) {
    ...
    final List<Map<String, dynamic>> resultadosFiltrados = productos.where((Map<String, dynamic> producto) {
      ...
      if (!coincideCategoria && producto['categoria'] != null) {
        final String categoriaProducto = producto['categoria'].toString().trim();
        final String categoriaProductoNormalizada = categoriaProducto.toLowerCase();
        final String filtroCategoriaNormalizadoLC = filtroCategoria.toLowerCase();
        
        // Intentar varias formas de comparación
        coincideCategoria = 
            categoriaProductoNormalizada == filtroCategoriaNormalizadoLC ||
            normalizarCategoria(categoriaProducto).toLowerCase() == filtroCategoriaNormalizadoLC;
        
        // Depuración mejorada
        if (debugMode) {
          if (!coincideCategoria) {
            debugPrint('❌ No coincide: "$categoriaProducto" ($categoriaProductoNormalizada) vs "$filtroCategoria" ($filtroCategoriaNormalizadoLC)');
          } else {
            debugPrint('✅ Coincidencia: "$categoriaProducto" con "$filtroCategoria"');
          }
        }
      }
    ...
    resultadosConDescuentoFiltrado.sort((Map<String, dynamic> a, Map<String, dynamic> b) {
  ...
  static List<String> extraerCategorias(List<Map<String, dynamic>> productos) {
    final List<String> categorias = productos
        .map((Map<String, dynamic> p) => p['categoria']?.toString().trim() ?? '')
  ...
  static List<String> combinarCategorias({
    ...
    if (categoriasProductos.length > 1) { // Si hay más que solo "Todos"
      return categoriasProductos;
    }
    
    // 2. Usar categorías de fallback si están disponibles
    if (categoriasFallback.isNotEmpty) {
      return <String>['Todos', ...categoriasFallback.where((String cat) => cat != 'Todos' && cat != 'Todas')];
    }
    
    // 3. Usar categorías de API si están disponibles
    if (categoriasApi.isNotEmpty) {
      categoriasApi.sort((Categoria a, Categoria b) => a.nombre.compareTo(b.nombre));
